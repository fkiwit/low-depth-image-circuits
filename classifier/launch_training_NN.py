...
from ray_helper import GCTuneCallback, ValidationAccuracyStopper
...
class RaySubmission:
   ...
   def __init__(
         ...
         scheduler_config: Dict = {"name": "fifo"},
         gpus: float = 1,
         ):
   ...
   def run(self):
      ...
      # stopper = ValidationAccuracyStopper()
      ...
      if self.scheduler_config["name"] == "fifo":
         ...
         scheduler = ASHAScheduler(
            ...
            metric="validation_accuracy",    # Or the actual metric key you report (could also be "accuracy", "validation_accuracy", etc.)
            mode="max",                      # Use "min" if you are minimizing the metric
            grace_period=10,                 # Let trials run for at least 5 epochs
...
class RaySubmissionCNN(RaySubmission):
   def __init__(
      ...
      super().__init__(dataset=dataset, fold=fold, compression_depth=compression_depth, gpus=1, scheduler_config=scheduler_config)
...
if __name__ == "__main__":
   ...
   parser.add_argument("--config_name", type=str, default="cnn", help="Name of the config to run (e.g., 'cnn', 'mpo', 'NonLinearVQC_shadow')")
   ...
   if config_name == "NonLinearVQC_shadow":
      ...
      submission = RaySubmissionTN(
         model_name="mpo",
         patched=True,
         # n_factors=tune.grid_search(list(range(1, 5))),
         ...
         compression_depth=tune.grid_search(list(range(1, 9))),
      ...
      submission = RaySubmissionCNN(
         ...
         weight_decay=0.0004115,
         epochs=50,
         fold=tune.grid_search([0, 1, 2, 3, 4]),
         compression_depth=tune.grid_search(list(range(9))),
         batch_size=100,
         scheduler_config={"name": "fifo"}
         )
   else:
